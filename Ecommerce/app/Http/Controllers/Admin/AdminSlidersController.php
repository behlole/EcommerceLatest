...
class AdminSlidersController extends Controller
{
	...
	public function addNewSlide(Request $request){
		...
		if($request->hasFile('newImage') and in_array($request->newImage->extension(), $extensions)){
			...
			$image->move('images/slider_images/', $fileName);
			$uploadImage = 'images/slider_images/'.$fileName; 
		}else{
			$uploadImage = '';
		}
	...
	public function updateSlide(Request $request){
		...
		if($request->hasFile('newImage') and in_array($request->newImage->extension(), $extensions)){
			$image = $request->newImage;
			$fileName = time().'.'.$image->getClientOriginalName();
			$image->move('images/banner_images/', $fileName);
			$uploadImage = 'images/banner_images/'.$fileName; 
		}else{
