	...
	public function updateMyProfile(Request $request){
		...
		if($request->hasFile('picture') and in_array($request->picture->extension(), $extensions)){
			...
			if(!preg_match($pattern, $verifyimg['mime']))
			{
				...
				$image->move('images/user_profile/', $fileName);
				$customers_picture = 'images/user_profile/'.$fileName; 
			}
		}	else{
			$customers_picture = "";
		}	
...
//		return redirect()->intended('/');
	...
	public function signupProcess(Request $request){
		...
		if($request->hasFile('picture') and in_array($request->picture->extension(), $extensions))
		{
			...
			if(!preg_match($pattern, $verifyimg['mime']))
			{
				$profile_photo = 'images/user_profile/default_user.png';
			} else {
			
				$fileName = time().'.'.$image->getClientOriginalName();
				$image->move('images/user_profile/', $fileName);
				$profile_photo = 'images/user_profile/'.$fileName; 
			}
		}else{
			$profile_photo = 'images/user_profile/default_user.png';
		}	
